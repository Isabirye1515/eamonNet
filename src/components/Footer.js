...
import {  Location, Phone, PhoneIncoming } from '@carbon/icons-react'; // Import location and phone icons
...
export default function Footer() {
  ...
  const icons = [
    {id:1,url:"https://www.facebook.com/eamon256", photo:facebook,name:"Facebook"},
    {id:2,url:"https://www.instagram.com/eamon_dreads_kampala/", photo:instagram, name:"Instagram"},
    {id:3,url:"https://www.tiktok.com/discover/eamon-dreadlocks-solution?lang=en", photo:tiktok, name:"Tiktok"}
  ]
  ...
  return (
    <Grid>
      ...
      <Column lg={8} sm={4} md={4}  className='footer' >
...
<a   href={url} target="_blank" rel="noreferrer" ><button className="btn btn-primary   mt-4"><Location  size={30} />Pack View Shopping Center</button></a>
     ...
     <a  className="btn btn-warning mt-4" href="tel:+256741099245"><PhoneIncoming size={30}  className="text-white" />+256741099245</a>
       {icons.map((icon)=>
    <p className='icon  d-flex ' key={icon.id} >
      <Link href={icon.url}  ><img src={icon.photo}  height={40} width={40} alt="icon" /></Link>{icon.name}
    </p>
    )}
    </Column>
